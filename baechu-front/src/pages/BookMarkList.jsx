    ...
    <div className='bookMark'>
      ...
      {currentPageData.map((e) => (
        <div key={e.id} className='bookMarkList'>
          {e.hasImage ?
            <img src={imageIcon} alt="이미지 첨부됨" className="bookMarkImg" /> :
            <img src={noImageIcon} alt="이미지 첨부 안됨" className="bookMarkImg" />
          }
          <div style={{ marginLeft: '20px' }}>
            <div className='bookMarkTitle'>글제목: {e.title}</div>
            <div className='bookMarkDetail'>{e.detail}</div>
          </div>
          <img src={bookMarkIcon} alt='북마크' className='bookMarkImg' onClick={{}}/>
        </div>
      ))}
